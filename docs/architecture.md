...
- `.autopilot/bin/control-plane.mjs` is the zero-model per-project terminal dashboard. When the global dashboard opens it, a backwards-compatible inherited marker changes its navigation action to **Back to main dashboard**; a direct launch closes back to the shell. Both paths release terminal input without changing worker state. The global dashboard also adds color-aware arrow-selectable fleet actions and a bounded, dependency-free folder picker; color falls back to plain text and neither feature invokes a model or executes candidate project code while browsing.
