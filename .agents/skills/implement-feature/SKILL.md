...
4. List environment-variable names only; the controller derives changed files from Git.
5. Write exactly one `.autopilot/runtime/candidate.json` object:
   `{schema_version:1,task_id,attempt,status:"complete|blocked|failed",summary,environment_variables:[],blocker:null|{kind,message,required_action,resume_condition}}`
